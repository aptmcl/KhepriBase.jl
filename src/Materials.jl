...
The material() constructor is defined in Shapes.jl. The realize method
calls b_material, which cascades through decreasing parameter tiers
(Full PBR → Standard PBR → Basic PBR → Color only) until a
backend-supported level is reached. See Backend.jl for the cascade.
=#
