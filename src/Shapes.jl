...
with_material_as_layer(f::Function, b::Backend, m::Material) =
  use_material_as_layer(b) ?
    let cur_layer = b_current_layer_ref(b),
        new_layer = ref_value(b, material_layer(m))
      cur_layer == new_layer ?
        f() :
        begin
...
#=
A standard material follows Filament's standard PBR model.
See Materials.jl for the full Filament property documentation.
The realize method (auto-generated by @defproxy) calls b_standard_material,
which is defined in Materials.jl and delegates to b_new_material.
=#
@defproxy(standard_material, Material, name::String="Material",
...
# BackendDefault() means "use the backend's default material" → b_get_material → void_ref.
# nothing means "no override" → use PBR parameters via b_standard_material.
...
# b_new_material get meaningful visual results out of the box.
...
export material_color, material_name, material_layer

# Color representative of a material's appearance
material_color(m::StandardMaterial) = m.base_color
material_color(m::MaterialInLayer) = m.layer.color
material_color(::Material) = rgba(0.5, 0.5, 0.5, 1.0)

# Name of a material
material_name(m::StandardMaterial) = m.name
material_name(m::MaterialInLayer) = m.layer.name
material_name(::Material) = "Material"

# Derive a layer from a material (for material_as_layer mechanism).
# Note: material_layer(m::MaterialInLayer) is auto-generated by @defproxy as a field accessor.
material_layer(m::StandardMaterial) = layer(m.name, true, m.base_color)
material_layer(::Material) = layer("Default")
